...
@Injectable()
export class ObtenerMetricasGeneralesUseCase {
  constructor(
  ...
  async ejecutar() {
    ...
    const datosCrudos = await this.dashboardRepositorio.obtenerMetricasGenerales();

    // Calcular métricas con el servicio de dominio
    return this.metricasDashboardService.calcularMetricasGenerales(datosCrudos);
  }
}
